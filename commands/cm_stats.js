...
module.exports = {
    ...
    async execute(message) {
        ...
        if(!("callsign_column_name" in guildData["career_mode_airtable_connection"])){
            message.channel.send("Your career mode airtable config is out of order. Keys are missing");
            return;
        }
        let careerModeData = await careerHelper.fetchCareerModeLogs(guildData["career_mode_airtable_connection"]);
        if(careerModeData === []){
        ...
        let record = await utils.filterRecordByCallsign(careerModeData,guildData["career_mode_airtable_connection"]["callsign_column_name"], callsign);
        if(record === {}){
            message.channel.send("Sorry your record could not be found in airtable!!");
            return;
        }
        let responseMessage = await messageCreator.createCmStatsMessage(record, guildData["career_mode_airtable_connection"]["fields"],guildData["career_mode_airtable_connection"]["callsign_column_name"])
